...
import type { Experience } from "../../../data/portfolio.ts";
...
const ExperienceItem = memo(function ExperienceItem({ experience, index }: ExperienceItemProps) {
  return (
    <article className="glass-panel rounded-xl p-6 glow-hover relative overflow-hidden">
      ...
      <div className="relative">
        <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4 mb-4">
          <div>
            ...
            <h3 className="font-headline-md text-xl text-primary">{experience.role}</h3>
        ...
        <ul className="space-y-3">
          {experience.bullets.map((bullet) => (
            <li key={bullet} className="flex gap-3 text-sm text-zinc-300 leading-relaxed">
              <Icon name="chevron_right" className="text-primary-container text-base mt-0.5" />
              <span>{bullet}</span>
            </li>
          ))}
        </ul>
